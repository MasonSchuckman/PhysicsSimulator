...
//camera system I made was causing the main update thread to
//wait for an update, thus lagging the system
public class SimpleFPSCamera extends Parent {
	private double x,y,z;
    public SimpleFPSCamera(double X,double Y,double Z) {
        x=X;
        y=Y;
        z=Z;
    	initialize();
    ...
    public void loadControlsForScene(Scene scene) {
        ...
        scene.addEventHandler(MouseEvent.ANY, me -> {
        	...
        	if (me.getEventType().equals(MouseEvent.MOUSE_PRESSED)) {
                ...
                } else if (me.isSecondaryButtonDown()&&!me.isAltDown()) {
                	double fov = getCamera().getFieldOfView();
                	getCamera().setFieldOfView(fov+mouseDeltaX/-10);
                } else if (me.isMiddleButtonDown()) {
                    /*
                     init panning?
                     */
    ...
    private void initializeCamera() {
        ...
        //MUST use this "affine" stuff to set the starting camera positon
        //i have no idea why, but using setTranslate() does some whack stuff.
		affine.setTx(x);
	    affine.setTy(y);
	    affine.setTz(z);
